...
public class RpcServerFactoryPBImpl implements RpcServerFactory {
  ...
  public Server getServer(Class<?> protocol, Object instance,
    ...
    Object service = null;
    try {
      // service即为XXXPBServiceImpl
      // instance参数即为XXXPBServiceImpl的real字段，以提供本地服务
    ...
    try {
      // method.invoke(null, protocolPBImpl)转化了hadoop的protocol为编译生成protobuffer的protocol（callBlockingMethod方法在不同的procotol下具备不同的匿名实现）
      // server调用路线：BlockingService.callBlockingMethod(args...) -> xxxServicePBImpl -> instance.xxx(args..) (instance实现了protocol的Service本地实例或其他实现了该protocol的本地实例)
